/* wiki-plugin-similarity - 0.4.3 - Mon, 06 Jul 2026 18:48:35 GMT */
...
`,it=(t,e)=>`<img class="sim-flag remote" src="${window.wiki.site(t).flag()}"
        title="${t} \u2014 score ${e.toFixed(3)}"
        data-site="${t}">`,z=(t,e,i,s)=>`<a class="sim-link" data-title="${i}" data-slug="${e}" data-site="${t}" href="#">${it(t,s)} ${i}</a>`,ot=(t,e)=>{let{mode:i,specs:s,threshold:o,limit:x,force:y,ghostUrl:v,label:b}=N(e?.text||"");if(i==="ghost")t.html(`
      <style>${j}</style>
      ...
      <div class="similarity" data-id="${e.id}">
        ...
        <div class="sim-status">Domains: ${p}</div>
        <div class="sim-results"></div>
      </div>`)}},at=(t,e)=>{let{mode:i,specs:s,threshold:o,limit:x,live:y,force:v,ghostUrl:b,thresholdSet:p}=N(e?.text||""),w=window.location.origin,n=t.find(".sim-status")[0],c=y?null:st(e),h=(d,g,r)=>{let l=[`${d} \u2014 ${g.toLocaleString()} pages across ${r} domains`];return p&&l.push(`threshold ${o}`),l.push(`limit ${x}`),y&&l.push("LIVE"),l.join(" \xB7 ")};t.on("dblclick",d=>{$(d.target).closest(".sim-input").length||window.wiki.textEditor(t,e)}),t.on("click",".sim-link",function(d){d.preventDefault();let g=$(this),r=d.shiftKey?null:t.parents(".page");window.wiki.doInternalLink(g.data("title"),r,g.data("site"))});let k=!s.length||s.length===1&&s[0]==="*"?"on farm":s.length===1?`on ${s[0]}`:"in domains",R=d=>d?` \xB7 cached ${nt(d)}`:"";if(i==="list"){let d=t.find(".sim-list")[0],g=s.length?s.join(","):"*",r=(l,m)=>{let a=l.reduce((u,f)=>u+(f.page_count||0),0);n.style.display="none",d.innerHTML=`<h3>Indexed Farm Domains</h3>
        <table>
        ...
        <p class="sim-count">${l.length} domains \u2014 ${a.toLocaleString()} pages${R(m)}</p>`};c?.domains?r(c.domains,c.ts):(async()=>{try{let l=`${w}/system/indexed-domains.json?pattern=${encodeURIComponent(g)}&limit=${x}`,m=await fetch(l);if(!m.ok)throw new Error(`indexed-domains failed: ${m.status}`);let a=await m.json();if(!a.length){n.textContent="No indexed domains found";return}r(a,null),I(e,{domains:a})}catch(l){n.textContent=`Error: ${l.message}`}})()}else if(i==="similar"){let d=t.find(".sim-results")[0],g=(r,l)=>{if(!r.length){n.textContent=`No similar pages found above threshold ${o}`;return}d.innerHTML=`<h3>Similar Pages</h3><ul>${r.map(({domain:m,slug:a,title:u,score:f})=>`<li>${z(m,a,u,f)}</li>`).join("")}</ul><p class="sim-count">${r.length} found ${k}${R(l)}</p>`,n.style.display="none"};c?.scored?g(c.scored,c.ts):(async()=>{try{let r=t.parents(".page"),l=r.find(".title").text().trim()||document.title,m=X(l),a=window.location.hostname,u=await A(s,w),f=u.reduce((C,T)=>C+T.pages.length,0);n.textContent=`Searching ${f.toLocaleString()} pages\u2026`;let L=await et(m,a);if(!L){n.textContent="Embedding page (not yet indexed)\u2026";let C=r.find(".item").map((T,D)=>$(D).text().trim()).get().filter(Boolean).join(`
`);L=await H(C||l,w)}let S=P(L,u,{threshold:o,limit:x,excludeSlug:m,excludeDomain:a});g(S,null),S.length&&I(e,{scored:S})}catch(r){n.textContent=`Error: ${r.message}`}})()}else if(i==="ghost"){let d=t.find(".sim-btn")[0],g=async()=>{if(!b){n.textContent="No URL \u2014 GHOST needs a page-json URL";return}d.disabled=!0,n.textContent="Fetching\u2026";try{let r=await fetch(b);if(!r.ok)throw new Error(`fetch failed: ${r.status}`);let l=await r.json();window.wiki.showResult(window.wiki.newPage(l),{$page:t.parents(".page")}),n.textContent=""}catch(r){n.textContent=`Error: ${r.message}`}finally{d.disabled=!1}};d.addEventListener("click",g)}else if(i==="build"){let d=t.find(".sim-btn")[0],g=async()=>{d.disabled=!0,n.textContent="Building index\u2026 (may take a while for large scopes)";try{let r=encodeURIComponent((s.length?s:["*"]).join(",")),l=await fetch(`http://api.localhost/build-index.json?domains=${r}&force=${v?1:0}`);if(!l.ok)throw new Error(`build-index failed: ${l.status}`);let m=await l.json();window.wiki.showResult(window.wiki.newPage(m),{$page:t.parents(".page")}),n.textContent=""}catch(r){n.textContent=`Error: ${r.message}`}finally{d.disabled=!1}};d.addEventListener("click",g)}else if(i==="report"){let d=t.find(".sim-input")[0],g=t.find(".sim-btn")[0],r=`Domains: ${s.length?s.join(", "):"*"}`;(async()=>{try{let m=await B(s.length?s:["*"],w),a=m.reduce((u,f)=>u+(f.page_count||0),0);r=h("Report ready",a,m.length),n.textContent=r}catch(m){n.textContent=`Domain listing unavailable: ${m.message}`}})();let l=async()=>{let m=d.value.trim();if(m){g.disabled=!0,n.textContent="Generating report\u2026";try{let a={query:m,domains:s.length?s:["*"],limit:x,live:y};p&&(a.threshold=o);let u=await fetch("http://api.localhost/search-report",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!u.ok)throw new Error(`search-report failed: ${u.status}`);let f=await u.json(),L=window.wiki.newPage(f);window.wiki.showResult(L,{$page:t.parents(".page")}),n.textContent=r}catch(a){n.textContent=`Error: ${a.message}`}finally{g.disabled=!1}}};g.addEventListener("click",l),d.addEventListener("keydown",m=>{m.key==="Enter"&&l()})}else if(i==="author"){let d=t.find(".sim-input")[0],g=t.find(".sim-btn")[0],r=t.find(".sim-results")[0],l=null;(async()=>{try{c||(n.textContent="Resolving domains\u2026"),l=await A(s,w);let a=l.reduce((u,f)=>u+f.pages.length,0);n.textContent=h("Author ready",a,l.length)}catch(a){n.textContent=`Load error: ${a.message}`}})();let m=async()=>{let a=d.value.trim();if(!(!a||!l)){g.disabled=!0,n.textContent="Embedding query\u2026",r.innerHTML="";try{let u=await H(a,w),f=P(u,l,{threshold:o,limit:x,excludeSlug:null,excludeDomain:null}),L=new Set,S=[];for(let{title:E}of f)L.has(E)||(L.add(E),S.push(E));let C=()=>Math.floor(Math.random()*18446744073709552e3).toString(16).padStart(16,"0"),T=S.map(E=>`- [[${E}]]`).join(`
`),D=[{type:"markdown",id:C(),text:`# Similar Pages

${T}`},{type:"markdown",id:C(),text:"# Reference Links"},...f.map(({domain:E,slug:J,title:K,score:W})=>({type:"reference",id:C(),site:E,slug:J,title:K,text:`score ${W.toFixed(3)}`}))],_=window.wiki.newPage({title:`${a} Results`,story:D,journal:[]});window.wiki.showResult(_,{$page:t.parents(".page")}),n.textContent=`${f.length} pages found`,I(e,{scored:f,query:a})}catch(u){n.textContent=`Error: ${u.message}`}finally{g.disabled=!1}}};g.addEventListener("click",m),d.addEventListener("keydown",a=>{a.key==="Enter"&&m()})}else{let d=t.find(".sim-input")[0],g=t.find(".sim-btn")[0],r=t.find(".sim-results")[0],l=null;c?.scored&&(d.value=c.query||"",r.innerHTML=c.scored.map(({domain:a,slug:u,title:f,score:L})=>`<div class="sim-result">${z(a,u,f,L)}</div>`).join("")+`<p class="sim-count">Top ${c.scored.length} for "${c.query||""}"${R(c.ts)}</p>`,n.textContent=""),(async()=>{try{c||(n.textContent="Resolving domains\u2026"),l=await A(s,w);let a=l.reduce((u,f)=>u+f.pages.length,0);n.textContent=h("Search ready",a,l.length)}catch(a){n.textContent=`Load error: ${a.message}`}})();let m=async()=>{let a=d.value.trim();if(!(!a||!l)){g.disabled=!0,n.textContent="Embedding query\u2026",r.innerHTML="";try{let u=await H(a,w),f=P(u,l,{threshold:o,limit:x,excludeSlug:null,excludeDomain:null});r.innerHTML=f.map(({domain:L,slug:S,title:C,score:T})=>`<div class="sim-result">${z(L,S,C,T)}</div>`).join("")+`<p class="sim-count">Top ${f.length} for "${a}"</p>`,n.textContent="",I(e,{scored:f,query:a})}catch(u){n.textContent=`Error: ${u.message}`}finally{g.disabled=!1}}};g.addEventListener("click",m),d.addEventListener("keydown",a=>{a.key==="Enter"&&m()})}};typeof window<"u"&&(window.plugins=window.plugins||{},window.plugins.similarity={emit:ot,bind:at});})();
