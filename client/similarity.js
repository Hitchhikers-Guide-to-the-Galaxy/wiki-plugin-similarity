/* wiki-plugin-similarity - 0.2.1 - Tue, 09 Jun 2026 22:28:50 GMT */
(()=>{var z={high:.78,medium:.68,low:.58},C=z.medium,j=10,F=t=>{let e=[],n=null,s=null,i="search",h=!1,d=(a,o)=>a===o||a.startsWith(o)&&/^[\s:]/.test(a.slice(o.length)),f=(a,o)=>a.slice(o.length).replace(/^\s*:?\s*/,"").trim();for(let a of t.split(`
`)){let o=a.trim();if(!o||o.startsWith("#"))continue;let w=o.toUpperCase();if(d(w,"LIVE")){h=!0;continue}if(d(w,"LIST")){!e.length&&i==="search"&&(i="list");continue}if(d(w,"SIMILAR")){let y=f(w,"SIMILAR").toLowerCase();n=z[y]||C,!e.length&&i==="search"&&(i="similar");continue}if(d(w,"THRESHOLD")){n=parseFloat(f(o,"THRESHOLD"))||C;continue}if(d(w,"LIMIT")){s=parseInt(f(o,"LIMIT"))||j;continue}e.push(o)}return{mode:i,specs:e,threshold:n??C,limit:s??j,live:h}},q=t=>t.includes("*")||t.includes("?"),N=t=>t.toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,""),E=new Map,P=async(t,e)=>{if(E.has(t))return E.get(t);let n=`${e}/system/indexed-domains.json?pattern=${encodeURIComponent(t)}`,s=await fetch(n);if(!s.ok)throw new Error(`indexed-domains failed: ${s.status}`);let i=await s.json();return E.set(t,i),i},_=async(t,e)=>{t.length||(t=[window.location.hostname]);let n=new Set,s=[];for(let i of t)if(i==="*"||q(i))for(let h of await P(i,e))n.has(h.domain)||(n.add(h.domain),s.push(h));else n.has(i)||(n.add(i),s.push({domain:i,page_count:null}));return s},I=new Map,V=()=>{let t=window.location.hostname;return t==="localhost"||t.endsWith(".localhost")||t==="127.0.0.1"},W=t=>V()?`${window.location.origin}/system/semantic-vectors.json?domain=${encodeURIComponent(t)}`:`${window.location.protocol}//${t}/system/semantic-vectors.json`,U=async t=>{if(I.has(t))return I.get(t);let e=await fetch(W(t));if(!e.ok)return[];let n=await e.json();return I.set(t,n),n},M=async(t,e)=>{let n=await fetch(`${e}/system/embed.json?text=${encodeURIComponent(t)}`);if(!n.ok)throw new Error(`embed failed: ${n.status}`);return(await n.json()).vector},J=async(t,e)=>{let s=(await U(e)).find(i=>i.slug===t);return s?s.vector:null},H=(t,e,{threshold:n,limit:s,excludeSlug:i,excludeDomain:h})=>{let d=[];for(let{domain:f,pages:a}of e)for(let{slug:o,title:w,vector:y}of a){if(o===i&&f===h)continue;let m=0;for(let p=0;p<t.length;p++)m+=t[p]*y[p];m>=n&&d.push({domain:f,slug:o,title:w,score:m})}return d.sort((f,a)=>a.score-f.score),d.slice(0,s)},R=async(t,e)=>{let n=await _(t,e);return(await Promise.all(n.map(async({domain:i})=>({domain:i,pages:await U(i)})))).filter(i=>i.pages.length>0)},A=t=>`sim-cache-${t}`,B=t=>{try{let e=JSON.parse(localStorage.getItem(A(t.id))||"null");return e?.text===(t.text||"")?e:null}catch{return null}},D=(t,e)=>{try{localStorage.setItem(A(t.id),JSON.stringify({text:t.text||"",ts:Date.now(),...e}))}catch{}},G=t=>{let e=Math.floor((Date.now()-t)/1e3);return e<60?`${e}s ago`:e<3600?`${Math.floor(e/60)}m ago`:e<86400?`${Math.floor(e/3600)}h ago`:`${Math.floor(e/86400)}d ago`},T=`
...
`,K=(t,e)=>`<img class="sim-flag remote" src="${window.wiki.site(t).flag()}"
        ...
        data-site="${t}">`,k=(t,e,n,s)=>`<a class="sim-link" data-title="${n}" data-slug="${e}" data-site="${t}" href="#">${K(t,s)} ${n}</a>`,Y=(t,e)=>{let{mode:n,specs:s,threshold:i,limit:h}=F(e?.text||"");if(n==="list"){let d=s.length?s.join(", "):"*";t.html(`
      <style>${T}</style>
      ...
      <div class="similarity" data-id="${e.id}">
        ...
        <p class="sim-count">${c.length} domains \u2014 ${l.toLocaleString()} pages${y(x)}</p>`};o?.domains?g(o.domains,o.ts):(async()=>{try{let c=`${f}/system/indexed-domains.json?pattern=${encodeURIComponent(p)}&limit=${h}`,x=await fetch(c);if(!x.ok)throw new Error(`indexed-domains failed: ${x.status}`);let l=await x.json();if(!l.length){a.textContent="No indexed domains found";return}g(l,null),D(e,{domains:l})}catch(c){a.textContent=`Error: ${c.message}`}})()}else if(n==="similar"){let m=t.find(".sim-results")[0],p=(g,c)=>{if(!g.length){a.textContent=`No similar pages found above threshold ${i}`;return}m.innerHTML=`<h3>Similar Pages</h3><ul>${g.map(({domain:x,slug:l,title:r,score:u})=>`<li>${k(x,l,r,u)}</li>`).join("")}</ul><p class="sim-count">${g.length} found ${w}${y(c)}</p>`,a.style.display="none"};o?.scored?p(o.scored,o.ts):(async()=>{try{let g=t.parents(".page"),c=g.find(".title").text().trim()||document.title,x=N(c),l=window.location.hostname,r=await R(s,f),u=r.reduce((v,S)=>v+S.pages.length,0);a.textContent=`Searching ${u.toLocaleString()} pages\u2026`;let b=await J(x,l);if(!b){a.textContent="Embedding page (not yet indexed)\u2026";let v=g.find(".item").map((S,O)=>$(O).text().trim()).get().filter(Boolean).join(`
`);b=await M(v||c,f)}let L=H(b,r,{threshold:i,limit:h,excludeSlug:x,excludeDomain:l});p(L,null),L.length&&D(e,{scored:L})}catch(g){a.textContent=`Error: ${g.message}`}})()}else{let m=t.find(".sim-input")[0],p=t.find(".sim-btn")[0],g=t.find(".sim-results")[0],c=null;o?.scored&&(m.value=o.query||"",g.innerHTML=o.scored.map(({domain:l,slug:r,title:u,score:b})=>`<div class="sim-result">${k(l,r,u,b)}</div>`).join("")+`<p class="sim-count">Top ${o.scored.length} for "${o.query||""}"${y(o.ts)}</p>`,a.textContent=""),(async()=>{try{o||(a.textContent="Resolving domains\u2026"),c=await R(s,f);let l=c.reduce((r,u)=>r+u.pages.length,0);a.textContent=`Ready \u2014 ${l.toLocaleString()} pages across ${c.length} domains`}catch(l){a.textContent=`Load error: ${l.message}`}})();let x=async()=>{let l=m.value.trim();if(!(!l||!c)){p.disabled=!0,a.textContent="Embedding query\u2026",g.innerHTML="";try{let r=await M(l,f),u=H(r,c,{threshold:i,limit:h,excludeSlug:null,excludeDomain:null});g.innerHTML=u.map(({domain:b,slug:L,title:v,score:S})=>`<div class="sim-result">${k(b,L,v,S)}</div>`).join("")+`<p class="sim-count">Top ${u.length} for "${l}"</p>`,a.textContent="",D(e,{scored:u,query:l})}catch(r){a.textContent=`Error: ${r.message}`}finally{p.disabled=!1}}};p.addEventListener("click",x),m.addEventListener("keydown",l=>{l.key==="Enter"&&x()})}};typeof window<"u"&&(window.plugins=window.plugins||{},window.plugins.similarity={emit:Y,bind:Q});})();
