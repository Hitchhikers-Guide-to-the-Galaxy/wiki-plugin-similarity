/* wiki-plugin-similarity - 0.1.9 - Tue, 09 Jun 2026 21:03:38 GMT */
...
`,N=(t,e)=>`<img class="sim-flag remote" src="${window.wiki.site(t).flag()}"
      ...
      <div class="similarity" data-id="${e.id}">
        ...
        <div class="sim-results"></div>
      </div>`)}},W=(t,e)=>{let{mode:n,specs:s,threshold:i,limit:d}=z(e?.text||""),r=window.location.origin,o=t.find(".sim-status")[0];if(t.on("dblclick",()=>window.wiki.textEditor(t,e)),t.on("click",".sim-link",function(a){a.preventDefault();let l=$(this);window.wiki.doInternalLink(l.data("title"),t.parents(".page"),l.data("site"))}),n==="list"){let a=t.find(".sim-list")[0],l=s.length?s.join(","):"*";(async()=>{try{let p=`${r}/system/indexed-domains.json?pattern=${encodeURIComponent(l)}&limit=${d}`,g=await fetch(p);if(!g.ok)throw new Error(`indexed-domains failed: ${g.status}`);let c=await g.json();if(!c.length){o.textContent="No indexed domains found";return}let m=c.slice(0,d),u=c.reduce((x,w)=>x+(w.page_count||0),0),f=m.length<c.length?`showing ${m.length} of ${c.length}`:`${c.length} domains`;o.style.display="none",a.innerHTML=`<h3>Indexed Farm Domains</h3>
        <table>
        ...
        <p class="sim-count">${f} \u2014 ${u.toLocaleString()} pages</p>`}catch(p){o.textContent=`Error: ${p.message}`}})()}else if(n==="similar")(async()=>{try{let l=t.parents(".page"),h=l.find(".title").text().trim()||document.title,p=U(h),g=window.location.hostname,c=await T(s,r),m=c.reduce((y,b)=>y+b.pages.length,0);o.textContent=`Searching ${m.toLocaleString()} pages\u2026`;let u=await q(p,g);if(!u){o.textContent="Embedding page (not yet indexed)\u2026";let y=l.find(".item").map((b,L)=>$(L).text().trim()).get().filter(Boolean).join(`
`);u=await k(y||h,r)}let f=D(u,c,{threshold:i,limit:d,excludeSlug:p,excludeDomain:g});if(!f.length){o.textContent=`No similar pages found above threshold ${i}`;return}let x=!s.length||s.length===1&&s[0]==="*"?"on farm":s.length===1?`on ${s[0]}`:"in domains",w=t.find(".sim-results")[0];w.innerHTML=`<h3>Similar Pages</h3><ul>${f.map(({domain:y,slug:b,title:L,score:M})=>`<li>${j(y,b,L,M)}</li>`).join("")}</ul><p class="sim-count">${f.length} found ${x}</p>`,o.style.display="none"}catch(l){o.textContent=`Error: ${l.message}`}})();else{let a=t.find(".sim-input")[0],l=t.find(".sim-btn")[0],h=t.find(".sim-results")[0],p=null;(async()=>{try{o.textContent="Resolving domains\u2026",p=await T(s,r);let m=p.reduce((u,f)=>u+f.pages.length,0);o.textContent=`Ready \u2014 ${m.toLocaleString()} pages across ${p.length} domains`}catch(m){o.textContent=`Load error: ${m.message}`}})();let c=async()=>{let m=a.value.trim();if(!(!m||!p)){l.disabled=!0,o.textContent="Embedding query\u2026",h.innerHTML="";try{let u=await k(m,r),f=D(u,p,{threshold:0,limit:d,excludeSlug:null,excludeDomain:null});h.innerHTML=f.map(({domain:x,slug:w,title:y,score:b})=>`<div class="sim-result">${j(x,w,y,b)}</div>`).join(""),o.textContent=`Top ${f.length} results for "${m}"`}catch(u){o.textContent=`Error: ${u.message}`}finally{l.disabled=!1}}};l.addEventListener("click",c),a.addEventListener("keydown",m=>{m.key==="Enter"&&c()})}};typeof window<"u"&&(window.plugins=window.plugins||{},window.plugins.similarity={emit:V,bind:W});})();
